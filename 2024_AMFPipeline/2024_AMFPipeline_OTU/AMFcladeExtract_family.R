...
#################################################################################
#### Determine which OTUs fall within the Pervestustaceae family clade ##########
#################################################################################
print('Loading RAxML best trees...')
treeFiles <- dir('./RAxMLfiles/', pattern ='RAxML_bestTree*') # make list of the tree files output from RAxML
for (tree in treeFiles) {
  ...
  print('---Extracting the AMF PER clade and identifying OTUs:')
  # Extract the PER clade
  AMFPER.clade <- extract.clade(mytree.rooted.preorder,AMFPER.node)
  ### Extract names of OTUs from the PER clade
  AMFPERnames<-c(AMFPER.clade$tip.label)
  ### Subset only those that start with 'OTU'
  hits<-AMFPERnames[grep("OTU",AMFPERnames)]
  if (tree==treeFiles[1]) { AMFPER.OTUs <- hits } # if this is the first tree in the list, make a new vector to store the hits
  else {AMFPER.OTUs <- append(AMFPER.OTUs, hits)} # otherwise, add the hits from this tree to the growing vector of AMF PER OTU names
  rm(mytree, mytree.rooted, mytree.rooted.preorder, tipDF, cladeStart, cladeStop, AMFPER.node, AMFPER.clade, AMFPERnames, hits) # clean up except for the AMF.OTUs vector
}

####### Load qiime2 output and prune out non-PER OTUs #######
print('Loading qiime2 output to be pruned - OTU table')
# Load OTU table and prune
otus <- read.delim('./otu97table_clean_BLASTonly.tsv',sep='\t',header=TRUE)
AMFPERotus <- subset(otus,X.OTU.ID %in% AMFPER.OTUs) # filter out non-PER OTUs using the list we generated above
# Write to file
write.table(AMFPERotus,file='./otu97table_clean_AMFPERonly.tsv',sep='\t',quote=FALSE,row.names=FALSE,col.names = TRUE)

print('Loading qiime2 output to be pruned - OTU rep. seqs')
# Load OTU rep seqs and prune
seqs <- read.FASTA('./otu97repseqs_clean_BLAST.fasta', type = 'DNA')
AMFPERseqs <- seqs[AMFPER.OTUs]
otherseqs <- seqs[setdiff(names(seqs),AMFPER.OTUs)]
# Write to file
write.FASTA(AMFPERseqs,'./otu97repseqs_clean_AMFPERonly.fasta')

print('AMF-PER LSU pipeline complete.')
print('AMF-PER OTU table and sequences have been written to file.')
