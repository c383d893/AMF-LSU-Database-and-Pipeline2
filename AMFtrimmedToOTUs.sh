...
blastn -db V15_LSUDB_3.23.21_AMFONLY -query R1.otu97repseqs_clean.fasta -outfmt 6 | sort -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 --merge | cut -f1 > S.BLAST.R1.otu97repseqs_clean.txt 

#Blast R2, extract top hit based on bit score, extract col 1, replace header with feature-id
blastn -db V15_LSUDB_3.23.21_AMFONLY -query R2.otu97repseqs_clean.fasta -outfmt 6 | sort -k1,1 -k12,12nr -k11,11n | sort -u -k1,1 --merge | cut -f1 > S.BLAST.R2.otu97repseqs_clean.txt
